...
const getData = () => {
    ...
    }).then(res => res.json()).then(data => {cities.value = data[0]; simplecities.value = data[1]})
        .catch((error) => {
...
<template>
    <VueFinalModal class="flex justify-center items-center"
        content-class="flex flex-col max-w-xl mx-4 p-4 bg-white md:w-1/3 h-1/2 border dark:border-gray-700 rounded-lg space-y-2">
        <div class="relative">
        ...
        <div class="px-6 py-6 lg:px-8">
            ...
            <div v-if="cities != null" class="h-56  grid grid-cols-2 mt-5">
                <div class="border-r-2 w-56">
                ...
                <div v-if="findcity" class="ml-4 overflow-y-auto w-56">
                    <div v-for="(city, n) in findcity" :key="n"
                        @click="form.city = city.title, form.cityid = city.id, $emit('confirm'), submit()"
                        class="hover:cursor-pointer hover:underline transition hover:-translate-x-1 duration-300 ml-2">
                        {{ city.title }}
                    </div>
                </div>

                <div v-else class="ml-4 overflow-y-auto w-56">
                    <div v-for="(city, n) in cities[regionname]" :key="n"
                        @click="form.city = city.title, form.cityid = city.id,  $emit('confirm'), submit()"
                        class="hover:cursor-pointer hover:underline transition hover:-translate-x-1 duration-300 ml-2">
                        {{ city.title }}
                    </div>
