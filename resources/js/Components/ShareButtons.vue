...
<template>
  <div class="flex">
    ...
    >
      <svg
        v-if="bundle.user_like"
        @click="submit(bundle.type, bundle.id, 'likes', 'down')"
        class="w-6 h-6"
        aria-hidden="true"
      ...
      <svg
        v-else
        @click="submit(bundle.type, bundle.id, 'likes', 'up')"
        class="w-6 h-6"
        aria-hidden="true"
        xmlns="http://www.w3.org/2000/svg"
        width="24"
    ...
    <!--share btn-->
    <span
      v-if="bundle.user_share"
      @click="setShare(bundle.type, bundle.url, bundle.id, 'nothing')"
      :data-popover-target="bundle.id + '-popover-click'"
    ...
    <span
      v-else
      @click="setShare(bundle.type, bundle.url, bundle.id, 'up')"
      :data-popover-target="bundle.id + '-popover-click'"
      data-popover-trigger="click"
      class="flex items-center text-gray-500 hover:text-gray-700 cursor-pointer"
    >
      ...
      <div class="px-3 py-2">
        ...
        <p @click="hrefCopy()" class="hover:underline cursor-pointer">
          <span v-if="iconCopy" class="flex">
            <svg
              class="w-[20px] h-[20px] mr-1 text-gray-800"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
        ...
        >
          <svg
            class="w-5 h-5 mr-1 fill-cyan-500 hover:fill-cyan-700"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            fill="currentColor"
            viewBox="0 0 24 24"
        ...
        >
          <svg
            ...
            x="0px"
            y="0px"
            class="w-5 h-5 mr-1 fill-amber-500 hover:fill-amber-600"
            viewBox="0 0 48 48"
          >
            <path
    ...
    <!--bookmarks-->
    <span
      v-if="bundle.user_bookmark"
      @click="submit(bundle.type, bundle.id, 'bookmarks', 'down')"
      class="flex items-center text-gray-500 hover:text-gray-700 cursor-pointer"
    >
      <svg
        class="w-6 h-6"
      ...
      >
        ...
        /></svg
      ><span v-if="bundle.bookmarks > 0" class="text-sm">{{
        bundle.bookmarks
      }}</span>
    </span>
