...
<template>
  <div class="mx-2">
    <h2 class="flex justify-center  mb-2 md:mb-10">
      <div
        @click="hideLinks = !hideLinks"
        class="text-center md:text-right md:w-2/3 font-bold border-b md:border-none"
      >
        юристы
      </div>
      <button
    ...
    <div :class="{ active: hideLinks, passive: !hideLinks }">
      <div v-for="(u, numb) in category" :key="numb" class="mx-3">
        <h2
          class="flex justify-start md:justify-end my-3 md:my-2 mx-3 md:mx-0 text-sm xl:text-base font-semibold"
        >
          <a
            v-if="cityUrl"
          ...
          <button
            v-if="u.mainhassecond"
            @click="hideusl = u.id"
            type="button"
            class="flex items-center justify-between ml-4 font-medium rtl:text-right text-gray-500"
            :data-accordion-target="'#accordion-flush-body-' + numb"
            :aria-expanded="u.url == main_usluga_url"
            :aria-controls="'accordion-flush-body-' + numb"
          >
            <svg
              v-if="u.mainhassecond.length > 0"
              data-accordion-icon
        ...
        <div
          v-if="u.mainhassecond"
          class="relative overflow-hidden transition-all max-h-0 duration-700 ease-out pr-5"
          :class="{
