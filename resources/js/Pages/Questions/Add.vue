...
function onInputheader(e) {
  ...
  if (e.target.value.length <= 100) {
    ...
    progressColor.value.width = 'Вопрос уже можно публиковать, но Вы можете рассказать подробнее'
  }
}
      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
              ...
              <div class="grid grid-cols-1 place-items-center w-full xl:w-3/4">
                <div class="mb-3 w-full col-span-2">
                  <meta itemprop="target" :content="'http://nedicom.ru/questions/similar/' + words.toString()
                    " />
                  <textarea itemprop="query" :onBlur="getQuestions" v-model="form.header" @input="onInputheader($event)"
                    maxlength="1155" name="header" required
                    ...
                    id="" rows="8" autofocus placeholder="Спрашивайте, юрист ответит обязательно..."></textarea>
                  <div class="my-1 w-full bg-gray-200 rounded-full h-1">
                    <div class="h-1 rounded-full" :class="progressColor.color" :style="{
                      width: progresswidth + '%',
                    }"></div>
                  </div>
                  ...
                  <div class="text-center items-center">
                    <div>
                      <div class="flex items-start my-5">
                        ...
                        <label for="remember"
                          class="ml-2 text-sm font-medium bg-white rounded-lg px-1 text-gray-900">Если таковые
                          содержаться в вопросе, даю согласие на
                          обработку
                          <Link href="/policy"
                      ...
                      <div class="flex items-start my-5">
                        ...
                        <label for="remember"
                          class="ml-2 text-sm font-medium bg-white rounded-lg px-1 text-gray-900">Мне есть 18 лет
                        </label>
                      </div>
                    </div>
          ...
          <div v-else class="my-20">
            ...
            <p class="text-gray-500 sm:text-xl text-center">
              Посмотрите
              <a :href="route('questions.url', [set.hasquestion.url])" class="text-blue-500 hover:underline">Ваш
                вопрос</a>.
            </p>
          </div>
        </div>
