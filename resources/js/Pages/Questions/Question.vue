...
function startPulse() {
  ...
  setTimeout(() => { isPulse.value = !isPulse.value }, 5000);
}
...
percent.value > 25 ? (color.value = 'bg-blue-200', message.value = "хорошо, но можно лучше") : null;
percent.value > 55 ? (color.value = 'bg-blue-400', message.value = "Вы почти у цели") : null;
percent.value == 100 ? (color.value = 'bg-blue-600', message.value = "Отлично, Ваш вопрос самый лучший") : null; 
</script>
        ...
        <div class="md:w-2/3 xl:w-1/2 sm:px-6 lg:px-4 py-3 px-3 md:px-0 mx-auto my-3 md:my-12 bg-white overflow-hidden"
          ...
          <div class="grid grid-cols-2">
            ...
            <div class="mx-4 flex items-center flex-col justify-center md:justify-end md:flex-row ">
              ...
              <div class="flex">
                <ShareButtons :bundle="set.question" :auth="set.auth" :class="{ 'animate-pulse': isPulse }" />
          ...
          <div v-if="set.question.uslugis_url" class="text-gray-900 text-center">Поискать юриста в категории -
            <h2 class="text-lg inline font-bold tracking-tight text-gray-900 hover:underline">
              <a :href="route('offer.main', [set.question.cities_url ? set.question.cities_url : 'all-cities', set.question.uslugis_url])"><span v-if="set.question.uslugis_usl_name">{{
                set.question.uslugis_usl_name
                  }}</span>
                <span v-if="set.question.cities_title"> - {{ set.question.cities_title
                  }}</span>
              </a>
            </h2>
          </div>
          ...
          <div class="px-10">
            <ol class="relative text-gray-500 border-l border-gray-200">
              ...
              <li class="mb-10 ml-6">
                ...
                <span v-else
                  class="absolute flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full -left-4 ring-4 ring-white">1</span>

                <h3 class="font-medium leading-tight text-2xl">Выберите
                  категорию</h3>

                <p v-if="set.question.uslugis_usl_name" class="text-sm">Категория определена</p>
                <p v-else class="text-sm">Отправим вопрос юристам с нужной специализацией</p>

                <div class="" v-if="set.question.user_id == authid || authid == 1 || !set.question.uslugis_usl_name">
              ...
              <li class="mb-10 ml-6">
                ...
                <span v-else
                  class="absolute flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full -left-4 ring-4 ring-white">2</span>
                ...
                <div class="my-5" v-if="set.question.user_id == authid || authid == 1">
                  <button @click="callChildMethod()" type="button"
                    ...
                    <span v-if="set.question.cities_title">{{ set.question.cities_title }}</span>
              ...
              <li class="mb-10 ml-6">
                ...
                <span v-else
                  class="absolute flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full -left-4 ring-4 ring-white">3</span>

                <h3 class="font-medium leading-tight text-2xl">Запросите дежурного юриста</h3>
              ...
              <li class="mb-10 ml-6">
                ...
                <span v-else
                  class="absolute flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full -left-4 ring-4 ring-white">4</span>

                <h3 class="font-medium leading-tight text-2xl">Заполните профиль</h3>
                <p v-if="set.question.phone" class="text-sm">Профиль заполнен</p>
                <p v-else class="text-sm">Автор вопроса не рассказал о себе</p>


                <div v-if="!set.question.phone && set.question.user_id == authid" class="my-5">
              ...
              <!--share-->
              <li class="mb-10 ml-6">
                <span v-if="set.question.user_share"
                  class="absolute flex items-center justify-center w-8 h-8 bg-green-200 rounded-full -left-4 ring-4 ring-white">
                  <svg class="w-3.5 h-3.5 text-green-500 dark:text-green-400" aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <span v-else
                  class="absolute flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full -left-4 ring-4 ring-white">5</span>
                ...
                <span
                  v-if="set.question.user_share && set.question.uslugis_usl_name && set.question.phone && set.answers.length && set.question.cities_title"
                  class="absolute flex items-center justify-center w-8 h-8 bg-green-200 rounded-full -left-4 ring-4 ring-white">
                  <svg class="w-3.5 h-3.5 text-green-500 dark:text-green-400" aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
