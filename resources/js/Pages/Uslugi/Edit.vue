        ...
        <div class="flex justify-left p-5">
          <div class="mb-3 md:w-3/6">
            <form @submit.prevent="submit">
                ...
                <div v-if="user.isadmin == 1" class="flex flex-col mb-4">
                  ...
                  <div  class="flex justify-between mb-4">
                    ...
                    <div v-if="form.is_main !== true">
                      <input v-model="form.is_second" id="default-checkbox" type="checkbox"
                        class="w-4 h-4 ml-2 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                      <label for="default-checkbox"
                        class="mx-2 text-sm font-medium text-gray-900 dark:text-gray-300">Сделать услугу
                        вторичной</label>
                    </div>
                    <div>
                  ...
                  <div>
                    <label class="block mt-5 mb-2 text-sm font-medium text-gray-900">Выберите
                      автора</label>
                    <select v-model="form.lawyer" required
                      class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                      <option disabled value="">юрист</option>
                      <option v-for="lawyer in set.lawyers" :key="lawyer.id" v-bind:value="lawyer.id"
                        :selected="lawyer.id == set.uslugi.user_id">
                        {{ lawyer.name }}
                      </option>
                    </select>
                  </div>
              ...
              <div v-if="showMstGoOn == 'contacts'">
                ...
                <label for="address" class="block text-sm font-medium leading-6 text-gray-900">Адрес (отображается в
                  шапке и на странице услуги в разделе
                  "адреса")</label>
                ...
                <label for="dopadress" class="block text-sm font-medium leading-6 text-gray-900">Дополнительный адрес
                  (не отображается в шапке, только на
              ...
              <div v-if="showMstGoOn == 'social'">
                <label class="block mb-2 text-sm font-medium text-gray-900">Номер группы во вконтаке</label>
                <input v-model="form.vk" name="vk"
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                  placeholder="72406118" />
                <p id="helper-text-explanation" class="mt-2 text-sm text-gray-500 dark:text-gray-400">Как узнать <a
                    href="https://vk.com/faq18062" target="_blank" class="text-blue-600 hover:underline">номер
                    группы</a> в ВК</p>

                <label class="block mb-2 mt-5 text-sm font-medium text-gray-900">Номер группы в одноклассниках</label>
                <input v-model="form.ok" name="ok"
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                  placeholder="70000003109279" />
                <p id="helper-text-explanation" class="mt-2 text-sm text-gray-500 dark:text-gray-400">Как узнать <a
                    href="https://apiok.ru/ext/group" target="_blank" class="text-blue-600 hover:underline">номер
                    группы</a> в ОК</p>
              </div>
            ...
            <Review v-if="
              ...
              showMstGoOn == 'reviews' &&
              auth.isadmin
            " class="mt-5 pt-5" :mainuslugaid="set.uslugi.main_usluga_id" :uslugaid="set.uslugi.id"
              :admin="set.user.id" :errors="set.errors" />
            <!-- otziv -->
          </div>
        </div>
