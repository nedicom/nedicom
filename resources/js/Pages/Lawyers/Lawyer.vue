...
let mainbannerimgmobile =
  'url("https://nedicom.ru/' + set.lawyer.avatar_path + '")';
</script>
...
<template>
  ...
  <div class="min-h-screen">
    <div class="grid grid-cols-1 md:grid-cols-4 py-5">
      <div
        class="w-full flex justify-between md:flex-col md:text-xl md:mt-12 px-5 sticky top-0"
      >
        <div
          class="w-full flex justify-between md:flex-col md:text-xl md:mt-12 px-5 sticky top-0"
        >
          ...
          <a v-if="set.lawyer.reviews_sum_rating"
            class="flex justify-end md:min-w-full md:mx-5 md:p-3 my-1 mr-1 text-gray-600 focus:outline-none transition hover:translate-x-1 duration-100"
            href="#otzivi"
          >
      ...
      >
        ...
        <ReviewCarousel
          v-if="set.lawyer.reviews_sum_rating"
          :reviews="set.lawyer.reviews"
          :rating="Number(set.lawyer.reviews_sum_rating)"
          :reviewscount="set.lawyer.reviews_count"
          :lwrid="set.lawyer.id"
          :auth="set.auth"
          :errors="set.errors"
          :mainuslugaid="null"
