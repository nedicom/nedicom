...
<template>
  <Head title="{{ vars.article.header }}">
    ...
    <meta
      property="og:image"
      :content="'https://nedicom.ru/' + vars.article.practice_file_path"
    />
  ...
  <Body>
    <div
      class="flex justify-left text-gray-900 md:px-10"
      itemscope
    ...
    <meta v-if="vars.answers[0]" itemprop="commentCount" :content="vars.answers.length" />
      ...
      >
        <a
          :href="'https://wa.me/' + article.phone + '?text=Здравствуйте.'"
          type="button"
      ...
      <!-- call to action -->
      <div
        class="w-full py-6 md:w-3/4 md:py-12 md:px-20 2xl:w-1/2 flex lg:px-8 bg-white overflow-hidden"
      >
        <div class="px-6 bg-white overflow-hidden">
          <div v-if="vars.auth" class="my-3">
            <div
              v-if="vars.auth.id == article.userid || vars.auth.isadmin == 1"
            >
              <a
          ...
          <div v-if="usluga" class="my-9 text-lg">
            Найти юриста:
            <a
              v-if="vars.region && vars.usluga"
              :href="
                'https://nedicom.ru/uslugi/' +
                vars.region.url +
                '/' +
                vars.usluga.url
              "
            ...
            >
            <a
              v-else-if="vars.region"
              :href="'https://nedicom.ru/uslugi/' + vars.region.url"
              class="font-bold hover:underline"
              >{{ vars.region.title }}</a
            >
            <a
              v-else-if="vars.usluga"
              :href="'https://nedicom.ru/uslugi/all-cities/' + vars.usluga.url"
              class="font-bold hover:underline"
              >{{ usluga.usl_name }}</a
            >
          </div>
          ...
          <div
            class="text-justify text-lg text-gray-800 articlebody"
            v-html="article.body"
            itemprop="text"
          ></div>
