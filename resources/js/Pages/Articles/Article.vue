...
<template>
  ...
  <Body>
    <div class="flex justify-left text-gray-900 md:px-10" itemscope itemtype="https://schema.org/Article">
      ...
      <!-- call to action -->
      <div class="hidden h-96 md:w-1/4 2xl:w-1/4 md:grid grid-cols-1 place-content-center px-5 md:px-0">
        ...
        <div class="px-6 bg-white overflow-hidden">
          ...
          <div class="flex items-center justify-center text-xs md:text-sm">
            <div class="md:block hidden mr-6" itemprop="dateModified" :content="vars.article.updated_at">
              ...
              <div>
                {{ (vars.article.body.length / 1000).toFixed(0) }} мин
              </div>
            </div>
          ...
          <div v-if="usluga" class="my-9 text-lg">
            ...
            <a v-else-if="vars.region" :href="'https://nedicom.ru/uslugi/' +
              vars.region.url
              " class="font-bold hover:underline">{{ vars.region.title }}</a>
            <a v-else-if="vars.usluga" :href="'https://nedicom.ru/uslugi/all-cities/' +
              vars.usluga.url
          ...
          <div class="md:hidden md:h-96 md:w-1/4 grid grid-cols-1 place-content-center px-5">
            <a :href="'https://wa.me/79855582170?text=Здравствуйте. Меня заинтересовала статья.' +
              vars.article.header +
          ...
          <div class="flex justify-center">
            <Answer
              ...
              :question="vars.article" :authid="vars.authid" :type="'article'" :article_id="vars.article.id" />
          </div>

          <div class="flex justify-center" id="comment">
            <Answers class="sm:px-6 lg:px-4 mx-5 py-12 md:w-4/6 w-full" :answers="vars.answers"
              :question="vars.question ? vars.question.id : null" :authid="vars.authid" :type="'article'"
