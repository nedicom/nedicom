...
<template>
    <section>
        ...
        <p class="mb-6 mx-12 font-light text-gray-500 md:text-lg dark:text-gray-400">Возможно Ваше дело далеко не первое...</p>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-for="card in practice" :key="card.id" class="w-full flex justify-center p-10">
                <a :href="route('articles/url', card.url)" target="_blank"
                    ...
                    <div class="flex flex-col justify-between p-4 leading-normal">
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ card.header
                            }}</h5>
