...
<yml_catalog date="{{ date('Y-m-d H:i') }}">
    ...
    <shop>
        ...
        <sets>
            ...
            @if (count($set->hassecondoffers) > 0)
            @foreach ($set->hassecondoffers as $child)

            @if ($child->hassecondoffers)
            @foreach ($set->hassecondoffers as $test)
            {{ $test->id }}
            @endforeach
            @endif
            <set id="s{{ $child->id }}">
