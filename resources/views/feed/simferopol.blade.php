        ...
        <offers>
            @foreach ($offers as $offer)
                ...
                <offer id="offer{{ $offer->id }}">
                    ...
                    @if ($offer->second_usluga_id)
                        <set-ids>s{{ $offer->second_usluga_id }}</set-ids>
                    @else<set-ids>s{{ $offer->main_usluga_id }}</set-ids>
                    @endif
                    <picture>@php echo url('/') @endphp/{{ $offer->file_path }}</picture>
                    <description>{{ $offer->usl_desc }}</description>
                    <adult>false</adult>
                    <expiry>P5Y</expiry>
                    @if ($offer->count_review > 0)
                    ...
                    <param name="Ссылка на телефон">{{ $offer->phone }}</param>
                </offer>
            @endforeach
        </offers>
    </shop>
